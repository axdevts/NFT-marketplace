...
contract EcchiCoin is Context, IERC20, Ownable {
    ...
    address private _rewardFeeAddress =
        address(0xBd6a01850AcA79bF0891A1F81aadd446239b9dE2);

    uint256 public _marketingFee = 1;
    uint256 private _previousMarketingFee = _marketingFee;
    address private _marketingFeeAddress =
        address(0x69b80f3004617CfCf3f705A93a7a90862440B0BD);

    uint256 public _devFee = 2;
    uint256 private _previousDevFee = _devFee;
    address private _devFeeAddress =
        address(0x848C9F6762E998408758bC8EdF58f704684d9f1a);
    /////////////////////////////////////////////////////////////////////
     ...
     * @param devFee The new dev fee percentage
     */
    function setNewFees(
        uint256 taxFee,
    ...
    function setDevFee(uint256 devFee, address devFeeAddress)
        external
        onlyOwner
    {
        _devFee = devFee;
     ...
     * @param contractTokenBalance Contract balance
     */
    function swapAndLiquifyUSDT(uint256 contractTokenBalance)
        private
        lockTheSwap
    {
        // split the contract balance into halves
        ...
        // has been manually sent to the contract
        // uint256 initialBalance = address(this).balance;
        uint256 initialUSDTBalance = BUSDAddress.balanceOf(address(this));
        ...
        // how much ETH did we just swap into?
        // uint256 newBalance = address(this).balance.sub(initialBalance);
        uint256 newUSDTBalance = BUSDAddress.balanceOf(address(this)).sub(
            initialUSDTBalance
        );
     ...
     * @param tokenAmount amount of token to authorize pancakeRouter
     * @param USDTAmount amount of USDT did we just swap into
     * @return uint256 amount of tokens
        ...
        )
    {
        // approve token transfer to cover all possible scenarios
        _approve(address(this), address(pancakeRouter), tokenAmount);
        BUSDAddress.approve(address(pancakeRouter), USDTAmount);

        // add the liquidity
        (uint256 amountToken, uint256 amountUSDT, uint256 liquidity) = pancakeRouter
            .addLiquidity(
                address(this),
                address(BUSDAddress),
                tokenAmount,
    ...
    function _transferTeamTax(
        ...
        if (_isExcluded[_devFeeAddress]) {
            ...
            _rOwned[_devFeeAddress] = _rOwned[_devFeeAddress].add(
